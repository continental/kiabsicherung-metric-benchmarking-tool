...
"""
Unit test number_of_false_positives.
...
def test_number_of_false_positives_init():
    """
    Test number of false positives initialization
    """
    # arrange
    num_fp_processor = NumberOfFalsePositives()

    # assert
    assert num_fp_processor.identifier == 1030
    assert num_fp_processor.name == 'Number of False Positives'
    assert num_fp_processor.calculate_per_sample is True


def test_number_of_false_positives_emtpy_data():
    """
    Test computation of false positives with empty input.
    """
    # arrange
    num_fp_processor = NumberOfFalsePositives()
    annotation_data, prediction_data, matching = get_empty_data()
    # act
    ans = num_fp_processor.calc_global(annotation_data=annotation_data,
...
def test_number_of_false_positives_per_class_empty_data():
    """
    Test computation of false positives per class with empty input.
    """
    # arrange
    num_fp_processor = NumberOfFalsePositives()
    annotation_data, prediction_data, matching = get_empty_data()
...
def test_number_of_false_positives_per_class_per_sample_empty_data():
    """
    Test computation of false positives per class per sample with empty input.
    """
    # arrange
    num_fp_processor = NumberOfFalsePositives()
    annotation_data, prediction_data, matching = get_empty_data()
    # act
    ans = num_fp_processor.calc_per_sample(annotation_data=annotation_data,
                                           prediction_data=prediction_data,
                                           matching=matching,
                                           calculate_per_class=True)
    # assert
...
def test_number_of_false_positives_fixture_data():
    """
    ...
    # assert
    assert isinstance(ans, pd.DataFrame), "wrong return type"
    assert ans["total"][0] == 2, "wrong result"
    assert len(ans) == 1, "wrong number of rows"
    assert len(ans.columns) == 1, "wrong number of columns"


def test_number_of_false_positives_per_class_fixture_data():
    """
    Test computation of false positives per class with default arguments.
    """
    # arrange
    num_fp_processor = NumberOfFalsePositives()
    annotation_data, prediction_data, matching = get_test_data()
    ...
    assert isinstance(ans, pd.DataFrame), "wrong return type"
    assert ans["total"][0] == 2, "wrong result"
    assert ans["human"][0] == 1, "wrong result for human"
    assert ans["vehicle"][0] == 1, "wrong result for vehicle"
    assert len(ans) == 1, "wrong number of rows"
    assert len(ans.columns) == 3, "wrong number of columns"


def test_number_of_false_positives_per_sample_fixture_data():
    """
    Test computation of false positives per sample with default arguments.
    """
    # arrange
    num_fp_processor = NumberOfFalsePositives()
    annotation_data, prediction_data, matching = get_test_data()
...
def test_number_of_false_positives_per_class_per_sample_fixture_data():
    """
    Test computation of false positives per class per sample with default arguments.
    """
    # arrange
    num_fp_processor = NumberOfFalsePositives()
    annotation_data, prediction_data, matching = get_test_data()
